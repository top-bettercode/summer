...
class UnitConverter : ConditionalGenericConverter {
    ...
    override fun convert(
        ...
        return if ((`object` as String?).isNullOrBlank()) {
            null
        } else {
    ...
    companion object {
        ...
        fun smaller(
            number: Number,
            value: Int = 100,
            scale: Int = log10(value.toDouble()).toInt()
        ): Long {
        ...
        fun <T> smaller(
            ...
            val result =
                BigDecimal(number).multiply(BigDecimal(value)).setScale(scale, RoundingMode.HALF_UP)
        ...
        fun <T> larger(
            ...
            val result =
                BigDecimal(number.toString()).divide(BigDecimal(value), scale, RoundingMode.HALF_UP)
