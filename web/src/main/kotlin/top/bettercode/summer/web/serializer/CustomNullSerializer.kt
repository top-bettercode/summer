...
class CustomNullSerializer(
    private val writer: BeanPropertyWriter,
    ...
    private val isArray: Boolean =
        type.isArray || MutableCollection::class.java.isAssignableFrom(type) && !MutableMap::class.java
    ...
    override fun serialize(value: Any?, gen: JsonGenerator, provider: SerializerProvider) {
        ...
        if (defaultValue == null) {
            ...
            if (jsonUrl != null && `val` != null) {
                UrlSerializer(
                    jsonUrl.value, jsonUrl.urlFieldName, jsonUrl.extended,
                    jsonUrl.asMap,
                    jsonUrl.separator, jsonUrl.defaultValue, jsonUrl.mapper
                ).serialize(
                    `val`, gen,
                    provider
                )
    ...
    private fun serializeExtend(
        gen: JsonGenerator, fieldName: String,
        defaultEmpty: Boolean
    ) {
        val value =
            if (!extendedValue.isNullOrBlank()) extendedValue else if (defaultEmpty) "" else null
        val jsonCode = writer.getAnnotation(JsonCode::class.java)
        ...
        if (jsonUrl != null && jsonUrl.extended) {
            ...
            if (isArray) {
                gen.writeObjectField(
                    urlFieldName,
                    if (defaultEmpty) Collections.EMPTY_LIST else null
                )
            } else {
    ...
    fun serializeNull(gen: JsonGenerator, type: Class<*>, value: Any?) {
        ...
        } else if ((jacksonExtProperties.isNullObjectAsEmpty || writer.getAnnotation(JsonDefault::class.java) != null) && (type.classLoader != null || MutableMap::class.java.isAssignableFrom(
                type
            ))
        ) {
    ...
    companion object {
        fun support(type: Class<*>): Boolean {
            return type == String::class.java || type.isArray || (MutableCollection::class.java.isAssignableFrom(
                type
            )
