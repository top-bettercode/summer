        ...
        if (size > 0) {
            ...
            for (i in recipes.indices) {
                ...
                if (notMixMaterials!!.isNotEmpty()) {
                    ...
                    val str = notMixMaterials.stream()
                            .map { arr: Array<String>? -> StringUtils.arrayToDelimitedString(arr, "和") }
                ...
                for (material in materials) {
                    ...
                    if (reqName != null) {
                        ...
                        // 最小用量
                        value(sheet, r + m, cc++, reqName + ":" + limit.min, mergeRow)
                        // 最大用量
                        value(sheet, r + m, cc++, reqName + ":" + limit.max, mergeRow)
                    } else {
                        cc += 2
                    }
                    if (liquidAmmonia == matrialName) {
                        ...
                        val limname = if (reqData.isLimitLiquidAmmonia) liquidAmmoniaMaterialRatioMap!!.keys.stream()
                                .filter { materialNameFragment: String? -> ReqData.isNeedLiquidAmmon(materialNameFragment, matrialName) }
                                .findFirst()
                        ...
                        if (limname != null) {
                            ...
                            val liquiName = if (reqData.isLimitVitriol) vitriolMaterialRatioMap!!.keys.stream()
                                    .filter { s: String? -> matrialName!!.contains(s!!) }
                                    .findFirst()
                                    .orElse(null) else null
                            if (liquiName != null) {
