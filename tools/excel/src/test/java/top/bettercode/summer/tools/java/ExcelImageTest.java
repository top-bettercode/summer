...
//    Cell cell = sheet.createRow(2).createCell(1);
    ...
    public void testMergeExportWithImage() throws IOException {
        ...
        ExcelConverter<top.bettercode.summer.tools.java.DataBean, InputStream> excelConverter = from -> {
            try {
        ...
        ExcelField<top.bettercode.summer.tools.java.DataBean, ?>[] excelMergeFields = ArrayUtil.of(
                ExcelField.<top.bettercode.summer.tools.java.DataBean, Integer>index("序号"),
                ExcelField.of("编码", top.bettercode.summer.tools.java.DataBean::getIntCode).mergeBy(top.bettercode.summer.tools.java.DataBean::getIntCode),
                ExcelField.of("编码B", top.bettercode.summer.tools.java.DataBean::getInteger).mergeBy(top.bettercode.summer.tools.java.DataBean::getInteger),
                ...
                ExcelField.image("图片1", excelConverter).mergeBy(top.bettercode.summer.tools.java.DataBean::getIntCode).width(10),
                ExcelField.image("图片2", excelConverter).width(10).height(40)
        );

        List<top.bettercode.summer.tools.java.DataBean> list = new ArrayList<>();
        for (int i = 0; i < 22; i++) {
            top.bettercode.summer.tools.java.DataBean bean = new top.bettercode.summer.tools.java.DataBean(i);
            list.add(bean);
        }
        for (int i = 22; i < 25; i++) {
            top.bettercode.summer.tools.java.DataBean bean = new top.bettercode.summer.tools.java.DataBean();
            bean.setIntCode(i);
            list.add(bean);
        }
