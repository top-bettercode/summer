...
import top.bettercode.summer.tools.excel.read.PropertySetter
...
import top.bettercode.summer.tools.excel.write.style.CellStyle.Companion.DEFAULT_FORMAT
...
@Deprecated("请使用CellSetter/CellGetter", replaceWith = ReplaceWith("CellSetter"))
class ExcelField<T, P : Any?> {
     ...
     * poi字段设置方法
     */
    var cellSetter: ((PoiExcel, CellData<T>) -> Unit)? = null
     ...
     * 获取实体属性
     */
    private lateinit var propertyGetter: Converter<T, P?>

    /**
     * 设置实体属性
     */
    private var propertySetter: PropertySetter<T, P?>? = null
     ...
     */
    @JvmOverloads
    fun date(format: String = DEFAULT_DATE_TIME_FORMAT): ExcelField<T, P> {
     ...
     * 从实体获取属性字段值
     */
    fun getter(propertyGetter: Converter<T, P?>): ExcelField<T, P> {
     ...
     * 向实体设置属性字段值
     */
    fun setter(propertySetter: PropertySetter<T, P?>): ExcelField<T, P> {
     ...
     * poi 自定义 单元格值
     */
    fun cellSetter(cellSetter: (PoiExcel, CellData<T>) -> Unit): ExcelField<T, P> {
    ...
    @Suppress("UNCHECKED_CAST")
    constructor(title: String, propertyGetter: Converter<T, P?>) {
        this.title = title
        this.propertyGetter = propertyGetter
        try {
        ...
        this.cellStyle.valueFormatting = CellStyle.defaultFormat(propertyType)
    }
    ...
    constructor(title: String, indexColumn: Boolean, formula: Boolean) {
        ...
        this.cellStyle.format(DEFAULT_FORMAT)
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun <T, P> of(title: String, propertyGetter: Converter<T, P?>): ExcelField<T, P> {
        ...
        fun <T, P> poi(
            title: String,
            propertyGetter: Converter<T, P?>,
            cellSetter: (PoiExcel, CellData<T>) -> Unit
        ): ExcelField<T, P> {
            return ExcelField(title, propertyGetter).cellSetter(cellSetter)
        }

        @JvmStatic
        fun <T, P> image(title: String, propertyGetter: Converter<T, P?>): ExcelField<T, P> {
