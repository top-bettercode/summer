 ...
 * 导入Excel文件
 */
@Deprecated("请使用ExcelReader", replaceWith = ReplaceWith("ExcelReader"))
class ExcelImport private constructor(`is`: InputStream) {
    ...
    fun <F, E> readRow(
        ...
        val rowErrors: MutableList<CellError> =
            ArrayList()
            ...
            try {
                ...
                rowErrors.add(
                    CellError(
                        ...
                        dateFormat = excelField.cellStyle.valueFormatting,
                        value = cellValue,
                        exception = e
                    )
                )
        ...
        return if (notAllBlank) {
            if (rowErrors.isNotEmpty()) {
                ...
                throw top.bettercode.summer.tools.excel.read.ExcelReaderException(
                    exception.message,
                    rowErrors,
                    exception
                )
