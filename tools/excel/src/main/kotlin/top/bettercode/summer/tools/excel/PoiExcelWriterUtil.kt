...
object PoiExcelWriterUtil {
    ...
    fun setPoi(poiCells: Map<String, List<ExcelCell<Any>>>,
        ...
        poiCells.forEach { (sheetName, cells) ->
            ...
            for (cell in cells) {
                ...
                (cell.excelField.poiSetter!!)(wb, poiCell, cell.entity)
            }
        }
    ...
    fun setImage(imageCells: Map<String, List<ExcelCell<*>>>,
                 inputStream: InputStream, outputStream: OutputStream) {
        val wb = XSSFWorkbook(inputStream)

        imageCells.forEach { (sheetName, cells) ->
            val sheet = wb.getSheet(sheetName)
