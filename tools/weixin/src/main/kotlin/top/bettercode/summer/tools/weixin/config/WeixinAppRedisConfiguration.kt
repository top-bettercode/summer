...
@ConditionalOnBean(IMiniprogramProperties::class, RedisConnectionFactory::class)
class WeixinAppRedisConfiguration(private val properties: IMiniprogramProperties) {

    @Bean
    fun miniprogramClient(redisConnectionFactory: RedisConnectionFactory): MiniprogramClient {
        return MiniprogramClient(properties, RedisWeixinCache(properties.cacheSeconds, MiniprogramClient.LOG_MARKER, redisConnectionFactory))
    }
