...
internal object Converter {
    ...
    fun tickC2A(
        ...
        lastTick: Tick? = null,
        volumeMultiple: Int? = null,
        marketStatus: MarketStatus = MarketStatus.UNKNOWN,
        onTimeParseError: (Exception) -> Unit
    ): Tick {
        val updateTime = try {
            LocalTime.parse("${tickField.updateTime}.${tickField.updateMillisec}")
        ...
        val bidPrice = arrayOf(
            formatDouble(tickField.bidPrice1),
            formatDouble(tickField.bidPrice2),
            formatDouble(tickField.bidPrice3),
            formatDouble(tickField.bidPrice4),
            formatDouble(tickField.bidPrice5)
        )
        val askPrice = arrayOf(
            formatDouble(tickField.askPrice1),
            formatDouble(tickField.askPrice2),
            formatDouble(tickField.askPrice3),
            formatDouble(tickField.askPrice4),
            formatDouble(tickField.askPrice5)
        )
        return Tick(
            code = code,
            time = updateTime,
            ...
            askPrice = askPrice,
            bidVolume = arrayOf(
                tickField.bidVolume1,
                tickField.bidVolume2,
                tickField.bidVolume3,
                tickField.bidVolume4,
                tickField.bidVolume5
            ),
            askVolume = arrayOf(
                tickField.askVolume1,
                tickField.askVolume2,
                tickField.askVolume3,
                tickField.askVolume4,
                tickField.askVolume5
            ),
            ...
            openInterestDelta = tickField.openInterest.toInt() - (lastTick?.todayOpenInterest
                ?: tickField.openInterest.toInt()),
            direction = if (lastTick == null) calculateTickDirection(
                lastPrice,
                bidPrice[0],
                askPrice[0]
            ) else calculateTickDirection(lastPrice, lastTick.bidPrice[0], lastTick.askPrice[0]),
            status = marketStatus,
     ...
     * 计算 Tick 方向
     */
    private fun calculateTickDirection(
        lastPrice: Double,
        bid1Price: Double,
        ask1Price: Double
    ): TickDirection {
    ...
    fun securityC2A(
        insField: CThostFtdcInstrumentField,
        onTimeParseError: (Exception) -> Unit
    ): SecurityInfo? {
        return try {
            val type = when (insField.productClass) {
                ...
                jctpConstants.THOST_FTDC_PC_SpotOption,
            ...
            if (type == SecurityType.UNKNOWN) null else {
                SecurityInfo(
                    ...
                    openDate = parseDate(insField.openDate),
                    expireDate = parseDate(insField.expireDate),
                    endDeliveryDate = parseDate(insField.endDelivDate),
    ...
    private fun parseDate(date: String?): LocalDate? =
        if (date.isNullOrBlank()) null else LocalDate.parse(date, DateTimeFormatter.BASIC_ISO_DATE)
    ...
    fun orderC2A(
        ...
        return Order(
            ...
            orderId,
            "${orderField.exchangeID}.${orderField.instrumentID}",
            orderField.limitPrice,
            null,
            orderField.volumeTotalOriginal,
            orderField.minVolume,
            directionC2A(orderField.direction),
            offsetC2A(orderField.combOffsetFlag),
            orderType,
            orderStatus,
            orderField.statusMsg,
            orderField.volumeTraded,
            orderField.limitPrice * orderField.volumeTraded * volumeMultiple,
            orderField.limitPrice,
            0.0,
            0.0,
            createTime,
            ...
            if (orderField.orderSysID.isNotEmpty()) orderSysId =
                "${orderField.exchangeID}_${orderField.orderSysID}"
        }
    }
    ...
    fun tradeC2A(
        ...
        val tradeTime = try {
            ...
            val updateTimeStr =
                "${date.slice(0..3)}-${date.slice(4..5)}-${date.slice(6..7)}T${tradeField.tradeTime}"
    ...
    fun positionC2A(
        ...
        positionField: CThostFtdcInvestorPositionField
    ): Position {
    ...
    fun futuresCommissionRateC2A(
        crField: CThostFtdcInstrumentCommissionRateField,
        code: String
    ): CommissionRate {
    ...
    fun futuresMarginRateC2A(
        mrField: CThostFtdcInstrumentMarginRateField,
        code: String
    ): MarginRate {
        return MarginRate(
            code = code,
