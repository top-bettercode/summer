...
internal class CtpMdApi(val config: CtpConfig, val kEvent: KEvent, val sourceId: String) {
     ...
     */
    fun querySubscriptions(useCache: Boolean, extras: Map<String, String>?): List<String> =
    ...
    suspend fun subscribeMarketData(
        ...
        val filteredCodes =
            if (extras?.get("isForce") != "true") codes.filter { it !in subscriptions } else codes
    ...
    private inner class CtpMdSpi : CThostFtdcMdSpi() {
        ...
        override fun OnRtnDepthMarketData(data: CThostFtdcDepthMarketDataField) {
            ...
            val newTick = Converter.tickC2A(
                code,
                data,
                lastTick,
                tdApi.instruments[code]?.volumeMultiple,
                tdApi.getInstrumentStatus(code)
            ) { e ->
                postBrokerLogEvent(
                    LogLevel.ERROR,
                    "【CtpMdSpi.OnRtnDepthMarketData】Tick updateTime 解析失败：$code, ${data.updateTime}.${data.updateMillisec}, $e"
                )
            }
